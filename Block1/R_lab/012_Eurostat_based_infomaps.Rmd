...
* Use of `{eurostat}` R package - API to access open data from [Eurostat](http://ec.europa.eu/eurostat)  
* data search  
* download   
* manipulation   
* visualization   
...
# Empirical example: unemployment data
...
```{r, message=F}
# https://ec.europa.eu/eurostat/databrowser/view/lfst_r_lfu3rt/default/table?lang=en
Un_DF <- get_eurostat("lfst_r_lfu3rt", time_format = "num") # the simplified time format
dim(Un_DF)
str(Un_DF)
UnemploymentDF <- Un_DF %>% 
...
```{r, echo=T, message=F}
Un_DF_lab <- label_eurostat(Un_DF, fix_duplicated = T)
```
...
```{r, warning=F}
kable(cbind(as.character(unique(Un_DF$isced11)),as.character(unique(Un_DF_lab$isced11))))
kable(cbind(as.character(unique(Un_DF$age)),as.character(unique(Un_DF_lab$age))))
kable(cbind(as.character(unique(Un_DF$sex)),as.character(unique(Un_DF_lab$sex))))
kable(cbind(as.character(unique(Un_DF$geo)),as.character(unique(Un_DF_lab$geo)))[1:17,])
...
```{r, eval=FALSE}
write.csv(Un_DF, "datasets/Unemployment.csv", row.names = F)
```
...
## Time path unemployment plot for selected countries 

Using the `Un_DF`, perform the following data handling (mostly filtering):  
...
Un_DF %>% 
  ...
  ggplot(aes(x = TIME_PERIOD, y = values, colour = geo))+ # plot filtered data
    ...
    ggtitle("Unemployment rates: country-level time paths")+ # Define main title
...
## Choropleth (infomap) of unemployment

We continue with the previously used `Un_DF` Unemployment dataset as in **Example 1** 

The following tasks (filtering) will be performed:  

* `Y15-74` i.e. age group *from 15 to 74 years*  
* Use data for 2018 and 2024  
...
```{r, echo=T, message=F}
countries <- c("AT","CZ","DE","HU","PL","SK")
# Download geospatial data from GISCO
geodata <- giscoR::gisco_get_nuts()
# Download state-borders separately
borders <- geodata |> 
  filter(CNTR_CODE %in% countries, LEVL_CODE == 0)
# Filter Unemployment dataset for the year 2017
myDF1 <- Un_DF %>% 
  filter(age == "Y15-74", 
         TIME_PERIOD %in% c(2018,2024), 
         sex == "T", 
         isced11 == "TOTAL") %>% 
  ...
  filter(NUTS0 %in% countries ) 
# Join Unemployment data with "map data"
map_data <- inner_join(geodata, myDF1)
# plot the data
ggplot()+ 
  ...
  geom_sf(data= borders, fill=NA, col="black",lwd=0.8)+
...
* Add Netherlands (`NL`) to the infomap.  
* Save the unemployment data (sf object) as `gpkg`
...
4. Retrieve data for the sector "Financial and insurance activities" for CZ, SK, SI, AT (NUTS0), years 2015 - 2024, in euro.
5. Plot the data using `ggplot` (time-path or infomap)
...
# Example 3: Non-spatial (time series) data handling


**Save Unemployment time series data into a `zoo` object**
