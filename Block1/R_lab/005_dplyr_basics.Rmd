...
Manipulating and analyzing data with `dplyr` using:
...
> * Use `group_by()` and `summarize()` to split a data frame into groups of observations, apply summary statistics for each group, and then combine the results.
...
* It pairs nicely with **`tidyr`** and other **`tidyverse`** packages and functionality.

* To learn more about **`dplyr`**, check 
...
```{r, message = FALSE,}
## load sample data
NUTS2.DF <- read.csv("data/NUTS2data.csv")
# summary(NUTS2.DF)
str(NUTS2.DF)
```
...
* With multiple packages loaded (time series-related) `dplyr::select()` can avoid conflicts in function names.
...
## Pipes `|>` (base) and `%>%` (magrittr)

- see [https://tidyverse.org/blog/2023/04/base-vs-magrittr-pipe/](https://tidyverse.org/blog/2023/04/base-vs-magrittr-pipe/) for explanation of the difference between `|>` and `%>%`
...
* <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>M</kbd> if you have a Windows-based PC  
...
* In the above code, we use the pipe to send the `NUTS2.DF` dataset first through `filter()` to keep rows where `Year` is $\leq$ 2011 AND `NUTS0 == "SI"`, then through `select()` to keep only the `Year`, `NUTS2` and `TotPopNr` columns. 
    * In our example, you cannot reverse the steps - if you 'select()' first, there is no variable `NUTS0` to use for filtering. 
...
* Retrieve GDP data for Austria & Czechia: filter for Austria (`"AT"`), Czechia (`"CZ"`) AND Year 2015,
...
```{r, message = FALSE,}
## load sample data
Unem <- read.csv("data/NUTS2data2.csv")
str(Unem)
```
...
* `left_join(sf_dataframe, dataframe, ...)` returns an `sf` object, 
...
# `summarize()` on spatial (`sf`) objects

* `dplyr::summarise()` can be used to unite (join) `sf` objects iniside a pipeline.  
* Nice alternative to the `sf::st_union()` command.
...
nc |> 
  summarize() |> 
  # st_geometry() skipped without effect on output
  plot()
```
