...
# Wide(r) and long(er) data formats - transformations using `{tidyr}`

---

## Goals for this scipt 

* Covert data from long(er) format to wide(r) format and back  
* `tidyr::pivot_longer()`  
* `tidyr::pivot_wider()`  

**Motivation**

> Data scientists, according to interviews and expert estimates, spend from 50 percent to 80 percent of their time in the mundane labor of collecting and preparing data, before it can be explored for useful information.  
...
Plot1 <- ggplot(data = GDPlong)+
  geom_line(aes(x = Year, y = GDP, color=State))+
  xlab("Year")+ylab("Artificial GDP data")+
  facet_grid(State~.)
Plot1

Plot2 <- ggplot(data = GDPlong)+
  geom_line(aes(x = Year, y = GDP, color=State))+
  xlab("Year")+ylab("Artificial GDP data")+
...
```{r}
GDPSpain <- read.csv("data/GDP_pc_Spain.csv")
...
```{r}
UnDF <- read.csv("data/UnExercise.csv")
